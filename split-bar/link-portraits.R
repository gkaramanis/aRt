...
img <- image_read("split-bar/images/audrey.jpg") %>%
...
ggplot(img_df) +
  ...
  geom_link(aes(x = x, y = y, xend = x, yend = y - bf * 10, alpha = stat(-index)), color = col_fill, size = 5) +
  scale_y_reverse() +
  scale_size_continuous(range = c(0, 3)) +
  coord_fixed(expand = FALSE) +
  theme_void() +
  theme(legend.position = "none", plot.background = element_rect(fill = col_bg, color = NA)) +
  ggsave("split-bar/plots/audrey-link.png", width = 8, height = 8 / img_ratio)
